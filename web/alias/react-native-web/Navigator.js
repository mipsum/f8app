...
class RouteContent extends Component {
  ...
  render() {
    ...
    if (this.props.params.routeIndex) {
      route = {
        index: parseInt(this.props.params.routeIndex, 10)
      }
    }
...
export class Navigator extends Component {
  ...
  constructor(props) {
    ...
    this.navigator.pop = () => {
      hashHistory.goBack()
      const returnRoute = this.navigator[this.navigator.length - 1]
  ...
  render() {
    return (
      <Router history={hashHistory}>
        <Route component={RouteContent} navigator={this.navigator}
