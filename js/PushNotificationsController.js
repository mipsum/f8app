...
class AppBadgeController extends Component {
  ...
  handleAppStateChange (appState) {
    if ('active' === appState) {
      this.updateAppBadge()
      if ('notifications' === this.props.tab) {
  ...
  componentDidUpdate (prevProps) {
    ...
    if ('notifications' === this.props.tab && prevProps.tab !== 'notifications') {
      this.eventuallyMarkNotificationsAsSeen()
    }
  }

  updateAppBadge () {
    if (this.props.enabled && 'ios' === Platform.OS) {
...
function select (store) {
  return {
    enabled: true === store.notifications.enabled,
