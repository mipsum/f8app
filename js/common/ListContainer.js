...
import {
  Animated, NativeModules, Dimensions, View, Platform, ActivityIndicator,
  findNodeHandle
} from 'react-native'
...
import hamburgerUnreadImg from './img/hamburger-unread.png'
...
class RelayLoading extends Component {
  render () {
...
export default class ListContainer extends Component {
  ...
  render () {
    ...
    if (!leftItem && 'android' === Platform.OS) {
      leftItem = {
        title: 'Menu',
    ...
    const backgroundShift = 1 === segments.length
      ? 0
  ...
  renderFloatingStickyHeader (stickyHeader: ?ReactElement) {
    ...
    var opacity = 0 === this.state.stickyHeaderHeigh ? 0 : 1
    var transform
  ...
  componentWillReceiveProps (nextProps: Props) {
    if ('number' === typeof nextProps.selectedSegment &&
  ...
  componentDidUpdate (prevProps: Props, prevState: State) {
    ...
    if (this.state.idx !== prevState.idx ||
      ...
      if (this._refs[prevState.idx] && this._refs[prevState.idx].getScrollResponder) {
        const oldScrollViewTag = findNodeHandle(
      ...
      if (this._refs[this.state.idx] && this._refs[this.state.idx].getScrollResponder) {
        const newScrollViewTag = findNodeHandle(
          this._refs[this.state.idx].getScrollResponder()
        )
        const pinnedViewTag = findNodeHandle(this._pinned)
