...
global.log = function (str, o) {
  ...
  let msg = `/---------------------\\
${str}${util.inspect(JSON.parse(JSON.stringify(o)), { showHidden: true, depth: null })}
\\---------------------/`
  ...
  fields: () => ({
    ...
    config: {
      ...
      resolve: () => Parse.Config.get()
        .then(res => {
          // sample of mocking data if not ready.
          // TODO: PLEASE dont include password on code. This is just a sample
          res.attributes.wifiNetwork = res.attributes.wifiNetwork || 'F8'
          res.attributes.wifiPassword = res.attributes.wifiPassword || 'codetoconnect'
